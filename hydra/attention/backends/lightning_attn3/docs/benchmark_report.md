...
3. **LA3-Parallel**: Parallel LA3 variant (tuning target)
...
- Current optimization/tuning focus: **LA3-Chunked** and **LA3-Parallel**.

### Tuning Notes (LA3-Chunked + LA3-Parallel)

- **LA3-Chunked**: optimized via runtime autotuning of the backward kernels with caching:
	- **Inter-backward**: `(CBLOCK, num_stages, num_warps)`
	- **Intra-backward**: `(CBLOCK, num_stages, num_warps)` under fixed `BLOCK=64`
- **LA3-Parallel**: optimized via runtime autotuning (BLOCK/CBLOCK/stages/warps) with caching.

- Benchmark visibility and micro-sweeps:
	- Normal benchmark prints the selected configs as `[cfg] LA3-Chunked inter: ...` and `[cfg] LA3-Chunked intra: ...`.
	- Inter micro-sweep: `python hydra/attention/backends/lightning_attn3/benchmark.py --micro-sweep --micro-sweep-n <N>`
	- Intra micro-sweep: `python hydra/attention/backends/lightning_attn3/benchmark.py --micro-sweep-chunked-intra --micro-sweep-n <N>`
	- Parallel micro-sweep: `python hydra/attention/backends/lightning_attn3/benchmark.py --micro-sweep-parallel --micro-sweep-n <N>`

- Cache behavior knobs:
	- `HYDRA_LA3_AUTOTUNE_KEY_BY_BH=1` (default): key Chunked caches by `bh=b*h` to avoid reusing a config tuned for a different occupancy.
	- `HYDRA_LA3_TUNING_CACHE=1` (default): persist best configs to disk as JSON to avoid re-tuning across processes.
	- `HYDRA_LA3_TUNING_CACHE_PATH=...`: override cache path (default `~/.cache/hydra/la3_tuning_cache.json`).
- Observed best LA3-Parallel configs on RTX 5090 (micro-sweep):
	- **N=1024 / 2048**: `BLOCK=128, CBLOCK=32, stages=2, warps=4`
	- **N=4096 / 8192**: `BLOCK=256, CBLOCK=64, stages=1, warps=4`
	- Note: `warps=2` can win at **N=2048**; the runtime grid includes it for `N<=2048`.
...
### Next Knobs (Recommended)

- **LA3-Chunked (highest leverage next)**: autotune `BLOCK` for the intra-backward kernel (compile-cost-sensitive). Current micro-sweeps show `BLOCK=64` winning at `N=4096` and `N=8192`, so the default grid is pruned to `BLOCK=64` and larger blocks are kept behind an aggressive env flag.
- **LA3-Chunked (accuracy of tuning)**: consider keying caches by `bh=b*h` (batch*heads), or measuring with the actual `bh`, since occupancy can change with `bh`.
- **LA3-Parallel**: expand the candidate grid slightly for small `N` (try `num_warps=2` in addition to 4/8) and validate via `--micro-sweep-parallel` before changing defaults.
